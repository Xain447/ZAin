...
class Space {
  ...
  static void init() {
    ...
    z = EdgeInsets.zero;
    h = EdgeInsets.symmetric(horizontal: AppDimensions.space(0.1));
    ...
    top = SizedBox(height: UserInterface.padding!.top);
    bottom = SizedBox(height: UserInterface.padding!.bottom);
  }

  static Widget xf([double no = 1]) => SizedBox(width: AppDimensions.space(no));

  static Widget yf([double no = 1]) => SizedBox(height: AppDimensions.space(no));
  ...
  static EdgeInsets all([double h = 0.5, double? v]) => EdgeInsets.symmetric(
    ...
    horizontal: AppDimensions.space(v ?? h),
  );
}
